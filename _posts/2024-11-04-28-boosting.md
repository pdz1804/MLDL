...
as classification tasks. ***Unlike bagging, which averages predictions,
...
![Boosting Demonstration](img/boosting.png){#fig:boosting
width="1\\linewidth"}
...
${}$\
The primary objectives of assigning weights to training instances
include:
...
![Boosting Algorithms](img/boosting-types.png){#fig:boosting-types
width="1\\linewidth"}
...
1.  **Initialization**: Initialize the model with a constant prediction:
    $$F_0(x) = \arg \min_\gamma \sum_{i=1}^N L(y_i, \gamma)$$ where
...
4.  **Update the Model**: Update the model by adding the predictions of
    ...
    $$F_t(x) = F_{t-1}(x) + \eta h_t(x)$$ where $\eta$ is the learning

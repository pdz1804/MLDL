      ...
      function (require, module, exports) {
        ...
        function search(crit) {
          ...
          return data.filter((item) => {
            // Check if the title contains the substring (case-sensitive)
            return item.title.includes(crit);
          });
        }
        ...
        function compile(data) {
          return options.template.replace(
            ...
            function (match, prop) {
              ...
              if (value !== undefined) {
                if (prop === "title") {
                  // Highlight query in the title
                  const query = options.searchInput.value.toLowerCase();
  ...
  */
          ...
          function initWithURL(url) {
            jsonLoader.load(url, function (err, json) {
              if (err) {
                console.error("Error loading JSON file:", err); // Add this for debugging
          ...
          //     if (query) {
          //       emptyResultsContainer(); // Clear previous results
          //       const results = repository.search(query); // Perform search
          //       render(results); // Render filtered results
          //     }
          //   });
          // }

          function registerInput() {
            options.searchInput.addEventListener("keydown", function (e) {
              if (e.key === "Enter") {
                // Check if the Enter key was pressed
                e.preventDefault(); // Prevent form submission
                const query = e.target.value.trim(); // Get and trim the query
                if (query) {
                  emptyResultsContainer(); // Clear previous results
                  const results = repository.search(query); // Perform search
                  render(results); // Render filtered results
          ...
          function render(results) {
            ...
            results.forEach((result) => {
              appendToResultsContainer(templater.compile(result));
            });
          }
